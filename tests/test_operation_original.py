...
def test_opsss(voter, steth_gauge, strategyProxy, ychad, currency,strategy,zapper,Contract, ldo, rewards,chain,vault, whale,gov,strategist, interface):
...
def test_zapper(voter, steth_gauge, strategyProxy, ychad, currency,strategy,zapper, chain,vault, whale,gov,strategist, interface):
    rate_limit = 1_000_000_000 *1e18
    debt_ratio = 10_000
...
def test_migrate(voter, currency, Strategy, strategyProxy, ychad, strategy, chain,vault, whale,gov,strategist, interface):
    rate_limit = 1_000_000_000 *1e18
    debt_ratio = 10_000
    vault.addStrategy(strategy, debt_ratio, rate_limit, 1000, {"from": gov})
    strategyProxy.setGovernance(gov,{"from":strategist})
    ...
    strategy2.setProxy(strategyProxy, {"from": gov})        # Must set the new strat up with the proxy
...
def test_reduce_limit(voter, steth_gauge, ychad, strategyProxy, currency, Strategy, strategy, chain, vault, whale,gov,strategist, interface):
    rate_limit = 1_000_000_000 *1e18
    debt_ratio = 10_000
    vault.addStrategy(strategy, debt_ratio, rate_limit, 1000, {"from": gov})
    strategyProxy.setGovernance(gov,{"from":strategist})
